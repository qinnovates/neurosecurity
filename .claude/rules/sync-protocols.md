...
**Script:** `src/scripts/update-automation-registry.mjs` | **CI:** `.github/workflows/update-registry.yml` (daily)
**When:** Workflow created/deleted/changed, script added to `src/scripts/`, hook modified in `.claude/hooks/`, automation status changes.
...
**Script:** `src/scripts/timeline-check.mjs` (use `--fix` to auto-update, `--dry-run` to preview) | **CI:** `.github/workflows/timeline-check.yml` (on push)
