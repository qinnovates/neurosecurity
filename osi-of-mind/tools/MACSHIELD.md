...
VPNs encrypt traffic at Layer 3+, but the attacks above happen at Layer 2 (the local network), below the VPN tunnel. A VPN cannot block ARP spoofing, hostname discovery, or NetBIOS enumeration. In the [QIF security model](https://github.com/qinnovates/neurosecurity/blob/main/osi-of-mind/whitepapers/QIF-TRUTH.md), VPNs operate at the **S3 band** (Application); macshield defends at **S1** (Analog Front-End).
