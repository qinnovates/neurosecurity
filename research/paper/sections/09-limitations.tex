...
\begin{enumerate}
  ...
  \item \textbf{Knight's Watch --- community deterrence mesh}: We propose an
        opt-in community awareness network that leverages the environmental
        sensors already present in vision restoration BCI devices. The concept
        draws from the established deterrence model of community watch programs:
        the knowledge that a community is observing changes the risk calculus
        for those who mean harm. Participating patients contribute anonymized,
        hashed environmental metadata---never raw imagery, never location, never
        identity---transmitted through NSP's post-quantum encryption. The target
        application is locating missing children and disrupting human trafficking
        networks. Design constraints include: (1)~explicit opt-in with revocable
        consent (cognitive liberty); (2)~COPPA compliance by architecture, not
        policy---no biometric data on minors is collected, stored, or transmitted;
        (3)~on-device processing via Runemate with no raw data exfiltration;
        (4)~blockchain-auditable participation history enabling patients to verify
        their contributions and law enforcement to verify alert provenance;
        (5)~inheritance of the full QIF security stack (NSP, Neurowall, Runemate)
        with no new trust assumptions. This concept requires legal analysis,
        ethics review, community governance design, and collaboration with
        anti-trafficking organizations before any implementation. The architecture
        is published openly to invite scrutiny and ensure that surveillance
        deterrence is built on privacy preservation, not surveillance expansion
  \item \textbf{EEG data pipeline for component validation}: We have assembled a
        ...
        Three specific research gaps are positioned for investigation:
        (a)~no ADHD-vs-ASD EEG classifier at clinical accuracy ($>$80\%) exists---published
        classifiers compare one disorder against typically developing controls, not against
        each other; (b)~no validated multi-paradigm EEG battery for 4-group differential
        diagnosis (ASD, ADHD, ASD+ADHD, typically developing) exists---the ABC-CT consortium
        battery is ASD-vs-TD only; and (c)~resting-state EEG alone is insufficient for ADHD
        discrimination---Shephard et~al.\ (2019) demonstrated that ADHD connectivity
        differences are invisible at rest, meaning task-state paradigms are required.
        A particularly promising result is the face processing double dissociation:
        in the same paradigm, the N170 component (170ms, early perceptual encoding)
        is impaired in ASD while the N400 component (400ms, semantic/affective integration)
        is impaired in ADHD, with the comorbid group showing both impairments. One task,
        two time windows, complete separation. The Healthy Brain Network dataset (3,000+
        subjects, multi-paradigm, transdiagnostic) provides a potential training set with
        approximately 66~ASD and 406~ADHD subjects under identical recording protocols.
        ...
        application. The security implication is direct: EEG recordings capable of
        differentiating neurodevelopmental conditions contain neural signature information
        sufficient for re-identification and clinical inference, making the security of
        diagnostic BCI pipelines a present design requirement, not a future concern.
        Documentation: \url{https://github.com/qinnovates/qinnovate/blob/main/shared/EEG-DATA-PIPELINE.md}
