...
\midrule
S1 & Near-Field / On-Device & Synthetic & Amplification, ADC, near-field EM coupling (0--10\,kHz) \\
S2 & Guided-Wave / Host-Local & Synthetic & Firmware, drivers, host compute, USB, decoding, BLE/WiFi baseband (10\,kHz--1\,GHz) \\
S3 & Far-Field / Wide-Area & Synthetic & RF transmission, directed energy, application layer (1\,GHz+) \\
\bottomrule
\end{tabularx}
\end{table}
...
front-end through host-side computation to radio transmission, organized by
propagation mode and spatial scale. S1 (Near-Field/On-Device) handles analog
signal conditioning and near-field electromagnetic coupling at the sensor level.
S2 (Guided-Wave/Host-Local) encompasses firmware, drivers, host compute, USB,
signal decoding, and BLE/WiFi baseband---all processes that operate over
guided-wave interconnects within the device-local environment. S3
(Far-Field/Wide-Area) covers radio frequency transmission, directed energy,
and application-layer communication at off-device ranges. Threats in this zone
are closest to traditional IT security and are most amenable to conventional
countermeasures.
