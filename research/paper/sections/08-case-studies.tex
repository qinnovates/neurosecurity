...
QIF-T0015 & P300 side-channel private data extraction     & 7.7 (High) & 3.3 (Low) \\
QIF-T0034 & BCI calibration data poisoning                & 8.2 (High) & 5.8 (Med) \\
QIF-T0050 & Covert neural signal decoding for surveillance & 7.1 (High) & 5.0 (Med) \\
...
\texttt{NISS:1.1/BI:C/CR:H/CD:H/CV:I/RV:P/NP:S} --- Score: 8.7 (High), PINS
flagged. The NISS vector captures that this technique causes critical biological
impact (BI:C), high cognitive reconnaissance (CR:H) and high cognitive
disruption (CD:H), covert consent violation (CV:I), partially irreversible
damage (RV:P), and structural neuroplastic changes (NP:S). The PINS flag
triggers mandatory safety review.
...
confidentiality breach; the actual impact is cognitive reconnaissance---covert
extraction of private mental content.

\paragraph{NISS extension.}
\texttt{NISS:1.1/BI:N/CR:H/CD:H/CV:E/RV:F/NP:N} --- Score: 3.3 (Low). No
biological impact (BI:N = 0), high cognitive reconnaissance (CR:H = 6.7) and
high cognitive disruption (CD:H = 6.7), explicit consent violation
(CV:E = 6.7), fully reversible (RV:F = 0), no neuroplastic effect (NP:N = 0).
The arithmetic mean is $(0 + 6.7 + 6.7 + 6.7 + 0 + 0)/6 = 3.3$. The NISS
score is significantly lower than CVSS because no physical harm occurs and the
attack is fully reversible---but the CR:H flag alerts BCI-specific teams that
private cognitive data is at risk.
...
\paragraph{NISS extension.}
\texttt{NISS:1.1/BI:L/CR:H/CD:H/CV:I/RV:T/NP:T} --- Score: 5.8 (Medium). Low
biological impact (BI:L = 3.3), high cognitive reconnaissance (CR:H = 6.7) and
high cognitive disruption (CD:H = 6.7, the device misinterprets intent), covert
consent violation (CV:I = 10.0, the patient doesn't know calibration was
corrupted), temporary reversibility (RV:T = 3.3), and temporary neuroplastic
changes (NP:T = 5.0). The arithmetic mean is
$(3.3 + 6.7 + 6.7 + 10.0 + 3.3 + 5.0)/6 = 5.8$.
...
\paragraph{NISS extension.}
\texttt{NISS:1.1/BI:N/CR:C/CD:C/CV:I/RV:F/NP:N} --- Score: 5.0 (Medium). No
biological impact (BI:N = 0), critical cognitive reconnaissance (CR:C = 10.0,
full thought decoding) and critical cognitive disruption (CD:C = 10.0), covert
consent violation (CV:I = 10.0), fully reversible (RV:F = 0), no neuroplastic
effect (NP:N = 0). The arithmetic mean is
$(0 + 10.0 + 10.0 + 10.0 + 0 + 0)/6 = 5.0$. Despite the moderate aggregate
score, the CR:C, CD:C, and CV:I flags distinguish this from ordinary data
exfiltration---these maximum-severity cognitive and consent violations demand
BCI-specific review.
