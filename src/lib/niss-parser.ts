...
const CR_VALUES: Record<CrCode, number | undefined> = {
  N: 0.0,
  L: 3.3,
  H: 6.7,
  C: 10.0,
...
const CD_VALUES: Record<CdCode, number | undefined> = {
  N: 0.0,
  L: 3.3,
  H: 6.7,
  C: 10.0,
...
const VALID_CR: ReadonlySet<string> = new Set(['N', 'L', 'H', 'C', 'X']);
 ...
 *
 * Expected format: `NISS:1.1/BI:H/CR:H/CD:C/CV:E/RV:P/NP:T`
 *
...
export function parseNiss(vectorString: string): NissVector {
  ...
  if (segments.length !== 7) {
    throw new Error(
      `Expected 7 segments (NISS:version + 6 metrics), got ${segments.length}: "${trimmed}"`
    );
  }
  ...
  const expectedOrder = ['BI', 'CR', 'CD', 'CV', 'RV', 'NP'];
  ...
  return {
    ...
    cr: metrics['CR'] as CrCode,
    cd: metrics['CD'] as CdCode,
...
export function validateNiss(vector: NissVector): string[] {
  ...
  if (!VALID_CR.has(vector.cr)) {
    errors.push(
      `Invalid CR code "${vector.cr}". Valid: ${Array.from(VALID_CR).join(', ')}`
    );
  }
  if (!VALID_CD.has(vector.cd)) {
    errors.push(
      `Invalid CD code "${vector.cd}". Valid: ${Array.from(VALID_CD).join(', ')}`
    );
  }
 ...
 *
 * @returns String like `NISS:1.1/BI:H/CR:H/CD:C/CV:E/RV:P/NP:T`
 */
 ...
 *
 * Without a profile: equal-weight average of assessed metrics (6 metrics).
 ...
 * If all six metrics are X, returns score 0.0 with severity 'none'.
 *
...
export function scoreNiss(
  ...
  const metrics: { key: keyof ProfileWeights; value: number | undefined }[] = [
    ...
    { key: 'cr', value: CR_VALUES[vector.cr] },
    { key: 'cd', value: CD_VALUES[vector.cd] },
 ...
 *
 * @param vectorString  e.g. `NISS:1.1/BI:H/CR:H/CD:C/CV:E/RV:P/NP:T`
...
export const CODE_LABELS: Record<string, Record<string, string>> = {
  ...
  cr: { N: 'None', L: 'Low', H: 'High', C: 'Critical', X: 'Undefined' },
