...
export const GLOSSARY_TERMS: GlossaryTerm[] = [
  ...
  },
  {
    ...
    term: 'Neural Oscillation Bands',
    type: 'concept',
    shortDef: 'Neural oscillation bands (delta, theta, alpha, gamma) operate at distinct frequencies and spatial scales.',
    fullDef: 'Neural oscillations propagate at different frequencies and spatial extents, governed by the wave equation (v = f x lambda). These band properties are relevant to BCI security because signals at specific frequencies must originate from spatially appropriate regions. The formalization of band-specific validation rules is future work.',
    relatedTerms: ['gamma-band', 'theta-band'],
    href: '/framework/#signal-integrity',
  ...
  },
  {
    ...
    term: 'TARA Taxonomy',
    type: 'architecture',
    shortDef: 'A threat classification system purpose-built for neural interfaces, extending prior BCI taxonomies with NISS scoring and dual-use classification.',
  ...
  },
  {
    ...
    fullDef: 'Proposed by Ienca & Andorno (2017). Neural data is not ordinary data. It is the substrate of thought itself. Mental privacy protects against unauthorized brain reading, thought surveillance, and neural data harvesting. QIF operationalizes MP with data-lifecycle specifications: cross-modal re-linking (correlating EEG with gait, voice, or keystrokes), anonymization failure, and informational disassociation. Implemented through transport variance detection, encryption, and the BCI Anonymizer.',
  ...
  },
  {
    ...
    fullDef: 'Proposed by Ienca & Andorno (2017). Protection from neural hacking, cognitive manipulation, and brain malware. QIF operationalizes MI with dynamical integrity specifications: protecting oscillatory rhythms, timing, and homeostatic equilibria from covert retuning attacks (gradual drift, baseline adaptation poisoning, neurofeedback falsification). Enforced through amplitude bounds, rate limiting, and the coherence metric.',
  ...
  },
  {
    ...
    fullDef: 'Neural data has 50+ year lifespans, making it vulnerable to Harvest-Now-Decrypt-Later (HNDL) attacks where adversaries collect encrypted neural data today and decrypt it when quantum computers mature. NSP uses post-quantum algorithms (ML-KEM, ML-DSA, and SLH-DSA/SPHINCS+) to protect against this threat.',
  ...
  // === Frequency Bands ===
  {
    ...
    fullDef: 'High-frequency neural oscillations (30-100 Hz) associated with attention, perception, and consciousness. Gamma has the smallest spatial extent (~1 cm) among cortical oscillation bands. Gamma coherence is central to Fries\' Communication Through Coherence theory (2005).',
  ...
  },
  {
    ...
    fullDef: 'Neural oscillations (4-8 Hz) associated with memory encoding, navigation, and hippocampal function. Theta spans approximately 5 cm in spatial extent.',
  ...
  // === DSM-5-TR Diagnostic Mapping (Entry 53) ===
  {
    ...
    type: 'concept',
    shortDef: 'A threat-modeling pipeline that maps a BCI technique to potential psychiatric impact categories: Technique \u2192 Band \u2192 Structure \u2192 Function \u2192 NISS + DSM-5-TR reference.',
    fullDef: 'A research model that traces how a BCI technique could produce psychiatric risk based on known neuroanatomy. The hourglass band indicates which brain structure is targeted. The structure indicates which cognitive function may be affected. NISS quantifies the severity (how much), and DSM-5-TR references the diagnostic category (what kind). The band is the Rosetta Stone connecting both systems. These mappings require clinical validation.',
  ...
  },
  {
    ...
    type: 'concept',
    shortDef: 'References DSM-5-TR diagnostic categories that correspond to each BCI technique\'s targeted neuroanatomy, using ICD-10-CM codes. For threat modeling purposes.',
    fullDef: 'Every TARA technique that touches a neural band (N1\u2013N7 or I0) is mapped to DSM-5-TR diagnostic categories via the Neural Impact Chain. Primary categories are the most closely associated with the technique\'s targeted neural mechanism based on published neuroanatomy; secondary categories are downstream risks. Each mapping includes a confidence level (established, probable, theoretical), risk class (direct, indirect, none), and the neural pathway chain. These are research references for threat modeling, not validated clinical predictions.',
  ...
  },
  {
    ...
    type: 'concept',
    shortDef: 'The link between NISS scores (how bad) and DSM diagnostic categories (what kind). Each NISS metric corresponds to a different diagnostic cluster.',
    fullDef: 'Maps NISS metrics to diagnostic risk domains: BI (Biological Impact) \u2192 Motor/Neurocognitive, CR (Cognitive Reconnaissance, read-side) \u2192 Cognitive/Psychotic, CD (Cognitive/Functional Disruption, write-side) \u2192 Cognitive/Psychotic, CV (Consent Violation) \u2192 Mood/Trauma, NP (Neuroplasticity) \u2192 Persistent/Personality, RV (Reversibility) \u2192 Chronicity modifier. This structural correspondence between security metrics and diagnostic categories is a research observation that requires clinical validation.',
